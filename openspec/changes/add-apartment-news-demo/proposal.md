...
- Wire GitHub OAuth via Sveltia's hosted auth proxy so the editor logs in with a GitHub account that has write access to the repo.
...
- New GitHub OAuth App registered against Sveltia's hosted auth proxy.
